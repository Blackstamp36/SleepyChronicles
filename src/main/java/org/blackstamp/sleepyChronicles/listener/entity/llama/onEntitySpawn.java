...
public class onEntitySpawn implements Listener {
    ...
    private void onEntitySpawn(CreatureSpawnEvent e) {
        ...
        if (entity.getEntitySpawnReason().equals(CreatureSpawnEvent.SpawnReason.NATURAL)
            ...
            if (global.getServerDay() >= 6 && (entity instanceof Zombie || entity instanceof Creeper) && !entity.getEntitySpawnReason().equals(CreatureSpawnEvent.SpawnReason.CUSTOM)) {
                if (r.nextInt(101) <= 1) {
                    aggresiveLlama.spawnEntity(entity.getLocation(), 1);
                    e.setCancelled(true);
