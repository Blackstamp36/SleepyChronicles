package org.blackstamp.sleepyChronicles.nms.v1_21_5_R01.entity.armor_stand;
...
public class homingProjectile extends ArmorStand {
    ...
    public homingProjectile(EntityType<? extends ArmorStand> entityType, Level level,
                            int projectileDamage, int lifetimeTicks, int delayTicks, LivingEntity target, LivingEntity shooter) {
    ...
    public void tick() {
        ...
        if(cM.checkBoundingBoxCollision(this.getBukkitLivingEntity(), target.getBukkitLivingEntity())){
            this.discard();
            handleProjectileImpact(target.getBukkitLivingEntity());
        }

    }

    private void handleProjectileImpact(org.bukkit.entity.LivingEntity damagedEntity){
        damagedEntity.damage(projectileDamage, shooter.getBukkitLivingEntity());
        damagedEntity.addPotionEffect(new PotionEffect(PotionEffectType.POISON,60,2));
        damagedEntity.addPotionEffect(new PotionEffect(PotionEffectType.WEAKNESS,60,0));
